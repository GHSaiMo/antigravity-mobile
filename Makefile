...
#
# 1. 命令行参数 (CLI Flags):
#    -port <端口号>        : 网关服务 HTTP/WebSocket 监听端口，默认 58900 (支持环境变量: GATEWAY_PORT)
#    -host <主机/IP>       : 监听地址，默认 "" 自动双栈监听本机所有 IPv4 与 IPv6 接口；
#                            若设为 127.0.0.1 则仅限本机访问 (支持环境变量: GATEWAY_HOST)
...
#    AUTH_STORE_PATH      : 设备凭据持久化存储路径 (默认 ~/.antigravity-mobile/auth_store.json)
